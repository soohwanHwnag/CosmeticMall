...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
    <link rel="stylesheet" href="/css/admin/adminOrderCancelBefore.css">
        ...
        <div class="orderCancelMain">
           <div class="pageName">
            ...
            <div class="orderCategory">
                <table>
                    <tr>
                        <td class="order_btn on" onclick="window.location.href='/admin/order/cancel/before'">결제 취소 전</td>
                        <td onclick="window.location.href='/admin/order/cancel/after'">결제 취소 완료</td>
                    </tr>
                </table>
            </div>
            <!-- 주문 카테고리 끝 -->
            <!-- 주문 리스트 -->
            <div class="orderList">
              <form action="orderCancelListMove" method="post">
                <table>
                    <tr>
                        <td>
            ...
            <div class="orderMove">
                <span>
                    <input type="submit" value="결제 취소 완료" onclick="clickEvent(); return false;">
                </span>
            </form>
            </div>
            <!-- 선택 항목 이동끝 -->
        ...
        <div class="page">
            <c:if test="${nowpage != 1}">
              <span ><a href="/admin/order/cancel/before?page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
               <span><a href="/admin/order/cancel/before?page=${i}">${i}</a></span>
              </c:forEach>
         ...
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${nowpage +1}">></a></span>
              </c:if>
        ...
        <c:if test="${not empty category && not empty value && empty orderDateEnd && empty orderDateStart}">
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/cancel/before?&orderListCategory=${category}&orderValue=${value}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/cancel/before?&orderListCategory=${category}&orderValue=${value}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/cancel/before?&orderListCategory=${category}&orderValue=${value}&page=${nowpage +1}">></a></span>
              </c:if>
         ...
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/cancel/before?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${nowpage +1}">></a></span>
              </c:if>

...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
    <link rel="stylesheet" href="/css/admin/adminOrderListDeliveryCompletion.css">
        ...
        <div class="orderMain">
           <div class="pageName">
            ...
            <!-- 주문 검색 -->
            <form action="deliveryCompletion" method="get">
                <div class="search">
            ...
            <div class="orderCategory">
                <table>
                    <tr>
                        <td class="order_btn" onclick="window.location.href='/admin/order/list/total'">전체</td>
                        <td onclick="window.location.href='/admin/order/list/waiting'">입금대기</td>
                        <td onclick="window.location.href='/admin/order/list/payCompletion'">결제완료</td>
                        <td onclick="window.location.href='/admin/order/list/deliveryReady'">배송준비중</td>
                        <td onclick="window.location.href='/admin/order/list/shipping'">배송중</td>
                        <td onclick="window.location.href='/admin/order/list/deliveryCompletion'">배송완료</td>
                    </tr>
                </table>
            </div>
            <!-- 주문 카테고리 끝 -->
            <!-- 주문 리스트 -->
            <div class="orderList">
              <form action="orderListMove" method="post">
                <table>
                    <tr>
                        <td>
                    ...
                    <tr>
                        ...
                        <td onclick="window.location.href='/admin/order/adminOrderView?order_idx=${list.order_idx}&order_member_idx=${list.order_member_idx}'">
                        ${list.order_number}
                        </td>
                        <td onclick="window.location.href='/admin/order/adminOrderView?order_idx=${list.order_idx}&order_member_idx=${list.order_member_idx}'">
                        ${list.product_name}
                        ...
                        <td onclick="window.location.href='/admin/order/adminOrderView?order_idx=${list.order_idx}'">
        ...
        <div class="page">
            <c:if test="${nowpage != 1}">
              <span ><a href="/admin/order/list/deliveryCompletion?page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
               <span><a href="/admin/order/list/deliveryCompletion?page=${i}">${i}</a></span>
              </c:forEach>
         ...
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&page=${nowpage +1}">></a></span>
              </c:if>
        ...
        <c:if test="${not empty category && not empty value && empty orderDateEnd && empty orderDateStart}">
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/list/deliveryCompletion?&orderListCategory=${category}&orderValue=${value}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/list/deliveryCompletion?&orderListCategory=${category}&orderValue=${value}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/list/deliveryCompletion?&orderListCategory=${category}&orderValue=${value}&page=${nowpage +1}">></a></span>
              </c:if>
         ...
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${nowpage -1}"><</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/order/list/deliveryCompletion?orderDateStart=${orderDateStart}&orderDateEnd=${orderDateEnd}&orderListCategory=${category}&orderValue=${value}&page=${nowpage +1}">></a></span>
              </c:if>

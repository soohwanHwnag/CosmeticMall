...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
    <link rel="stylesheet" href="/css/admin/adminEventListContinue.css">
        ...
        <!-- 검색창 끝 -->
         ...
         <div class="eventCategory">
            <table>
                <tr>
                    <td class="event_btn" onclick="window.location.href='/admin/board/eventlist/continue'">진행중인 이벤트</td>
                    <td onclick="window.location.href='/admin/board/eventlist/finish'">종료된 이벤트</td>
                </tr>
            </table>
        </div>
        <!-- 이벤트 카테고리 끝 -->
        ...
        <div class="eventList">
        <form action="eventListMove" method="post">
            <table>
                <tr>
                    <td>
        ...
        <!-- 버튼 시작 -->
        <div class="button">
            <button onclick="clickEvent(); return false;">선택 항목 종료</button>
             </form>
        ...
        <!-- 버튼 끝 -->
         ...
         <div class="page">
           <c:if test="${nowpage != 1}">
              <span><a href="/admin/board/eventlist/continue?eventSearchCategory=${category}&eventSearchValue=${value}&page=${nowpage -1}">1</a></span>
              </c:if>
              <c:forEach var="i" begin="${block_start}" end="${block_end}">
              <span><a href="/admin/board/eventlist/continue?eventSearchCategory=${category}&eventSearchValue=${value}&page=${i}">${i}</a></span>
              </c:forEach>
              <c:if test="${nowpage < block_total}">
              <span><a href="/admin/board/eventlist/continue?eventSearchCategory=${category}&eventSearchValue=${value}&page=${nowpage +1}">1</a></span>
              </c:if>
